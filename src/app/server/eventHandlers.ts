...
import { getWhisperOptions } from '../config/whisperConfig';

// Singleton instances
const audioProcessor = new AudioProcessor();
const sessionService = new SessionService();
const whisperService = new WhisperService(getWhisperOptions());
...
function setupEventListeners(io: Server, socket: Socket): void {
  ...
  socket.on(SocketEvent.SESSION_CREATE, (payload: SessionCreatePayload, callback) => {
    handleSessionCreate(socket, payload)
      .then((response) => {
        callback(response);
      })
      .catch((error) => {
  ...
  socket.on(SocketEvent.SESSION_DESTROY, ({ sessionId }: { sessionId: string }, callback) => {
    handleSessionDestroy(socket, sessionId)
      .then(() => {
...
async function handleSessionCreate(
  ...
  try {
    ...
    // Start WhisperX service if not already running
    if (!whisperService.isActive()) {
...
async function handleSessionDestroy(socket: Socket, sessionId: string): Promise<void> {
  ...
  if (socketSessionMap.size === 0) {
    // If no active sessions, stop the WhisperX service to save resources
...
function handleAudioChunk(io: Server, socket: Socket, payload: AudioChunkPayload): void {
  ...
  const audioForProcessing = audioProcessor.getAudioForProcessing(sessionId);
  if (audioForProcessing) {
    // Use WhisperX service to transcribe audio
    whisperService
      .transcribeAudio(audioForProcessing.audioData, audioForProcessing.sampleRate)
      .then((transcriptionResult) => {
        ...
        socket.emit(SocketEvent.TRANSCRIPTION_RESULT, resultWithSession);
      })
      .catch((error) => {
