...
export class SocketService {
  ...
  async createSession(deviceInfo?: Session['deviceInfo']): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      this.socket.emit(SocketEvent.SESSION_CREATE, { deviceInfo }, (response: { sessionId: string; error?: string }) => {
        if (response.error) {
  ...
  async endSession(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.socket.emit(SocketEvent.SESSION_DESTROY, { sessionId: this.sessionId }, (response: { success: boolean; error?: string }) => {
        if (response.error) {
          reject(new Error(response.error));
          return;
        }
  ...
  once<E extends SocketEvent>(event: E, callback: (payload: SocketEventPayloadMap[E]) => void): void {
    if (!this.socket) {
      return;
    }
  ...
  off<E extends SocketEvent>(event: E, callback?: (payload: SocketEventPayloadMap[E]) => void): void {
    if (!this.socket) {
      return;
    }
